...
private fun handleMergedPrAnalysis(
    ...
    reposAndPullRequestsLimits.map { (repoName, prPullLimit) ->
        ...
        var mergedPRs =
            closedPrs
                .filter { it.isMerged }
                .filter { notBeforeLimit == null || it.mergedAt.toInstant().isAfter(notBeforeLimit) }
        ...
        val timeToFirstReviewDurations =
            mergedPRs.map {
        ...
        println("Analyzing average time to first review for ${mergedPRs.count()}  PRs")
        ...
        val individualContributorStats =
            if (includeIndividualStats) {
                getIndividualStatistics(repo.listContributors().map { it.login }, mergedPRs)
            } else {
                emptyList()
            }
            ...
            val json =
                JsonMapper.writeValueAsString(
                    mapOf(
                        ...
                        "individualStats" to
                            individualContributorStats.map {
                                mapOf(
                                    "name" to it.author,
...
private fun calcDurationWithoutWeekends(
...
private fun printMergeStats(
    prefix: String,
    firstReviewDuration: Duration,
    mergeDuration: Duration,
) {
    println(
        "$prefix Time to First Review: ${firstReviewDuration.toDays()} days, " +
            "${firstReviewDuration.toHoursPart()} hours, ${firstReviewDuration.toMinutesPart()} minutes.",
    )
    println(
        "$prefix Time to Merge: ${mergeDuration.toDays()} days, " +
            "${mergeDuration.toHoursPart()} hours, ${mergeDuration.toMinutesPart()} minutes.",
    )
}

private fun getIndividualStatistics(
    contributorNames: List<String>,
    prs: List<GHPullRequest>,
    ...
    reposAndPullRequestsLimits.map { (repoName, prPullLimit) ->
        ...
        if (outputType.equals(JSON_OUTPUT_TYPE, ignoreCase = true)) {
            ...
            openPRs.forEach {
                ...
                println(
                    "PR: ${it.number} ${it.title} has been open for ${openDuration.toDays()} days, ${openDuration.toHoursPart()} hours.",
                )
            }
        }
    }
...
class GithubPrAnalyzer : CliktCommand() {
    ...
    private val pullRequestPullLimit: List<Int> by option("-l", "--pr-limit")
        ...
        .multiple(listOf(30))
        .help(
            "Limit the amount of PRs to analyze (default 10). Can be a list, where each " +
    ...
    override fun run() {
        ...
        val reposAndPullRequestsLimits =
            when (pullRequestPullLimit.size) {
                1 -> repositoryNames.map { it to pullRequestPullLimit.first() }
                repositoryNames.size -> repositoryNames.zip(pullRequestPullLimit)
                else ->
                    throw RuntimeException(
                        "'pr-limit' option must have a single provided argument or must the length of the 'repo-name' option.",
                    )
            }

        val notBeforeDateLimit: Instant? =
            notBeforeLimit
